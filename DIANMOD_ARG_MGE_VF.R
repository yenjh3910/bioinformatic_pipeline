...
arg_vf <- SHRiver.bin.ARGminer.bins.Covsel.id70 %>% filter(Bin %in% vfbin)
arg_novf <- SHRiver.bin.ARGminer.bins.Covsel.id70 %>% filter(!(Bin %in% vfbin))
mge_vf <- SHRiver.bin.MGE.bins.Covsel.id70 %>% filter(Bin %in% vfbin)
mge_novf <- SHRiver.bin.MGE.bins.Covsel.id70 %>% filter(!(Bin %in% vfbin))
...
for (i in 1:length(temp)){
  ...
  z$V5 <- z$V5-z$V4+1
  z$V4 <- 1
  z <- subset(z, V1 %in% arg_vf$qseqid)
  setwd("//wsl$/Ubuntu/home/yen/ARG_MGE_manually_gtf/")
  write.table(z, file=paste0(temp2[i],".vf.gtf.txt"), sep="\t",row.names = F,col.names = F,quote = F)
...
for (i in 1:length(temp)){
  ...
  z$V5 <- z$V5-z$V4+1
  z$V4 <- 1
  z <- subset(z, V1 %in% arg_novf$qseqid)
  setwd("//wsl$/Ubuntu/home/yen/ARG_MGE_manually_gtf/")
  write.table(z, file=paste0(temp2[i],".novf.gtf.txt"), sep="\t",row.names = F,col.names = F,quote = F)
}#remove 0kb file by myself
...
for (i in 1:length(temp)){
  ...
  z$V5 <- z$V5-z$V4+1
  z$V4 <- 1
  z <- subset(z, V1 %in% mge_vf$qseqid)
  setwd("//wsl$/Ubuntu/home/yen/ARG_MGE_manually_gtf/")
  write.table(z, file=paste0(temp2[i],".vf.gtf.txt"), sep="\t",row.names = F,col.names = F,quote = F)
}#remove 0kb file by myself
...
for (i in 1:length(temp)){
  ...
  z$V5 <- z$V5-z$V4+1
  z$V4 <- 1
  z <- subset(z, V1 %in% mge_novf$qseqid)
  setwd("//wsl$/Ubuntu/home/yen/ARG_MGE_manually_gtf/")
  write.table(z, file=paste0(temp2[i],".novf.gtf.txt"), sep="\t",row.names = F,col.names = F,quote = F)
}#remove 0kb file by myself
